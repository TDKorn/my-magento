...
>>> magento.logger.warning("Warning from my-magento") 

2022-06-17 00:16:06 WARNING  |[ MyMagento | my-magento ]|:  Warning from my-magento
```
...
#### Access a `Client` 's current logger via ```Client.logger``` or create a new one via ```Client.get_logger()```

- Note that ```get_logger()``` will return a new ```MagentoLogger``` instance but it does NOT automatically update the ```Client``` 

  - It's meant to be a shortcut to quickly create a new logger based off the ```Client```  (since the ```domain```/```username``` combination is used to configure the logger and log files)

  - Ex. to create a new logger with everything the same but a different log level

#### Alternatively, just use ```Client.logger.setup_logger()``` to modify logger settings in place
...
The package logger/log file is based on the value of ```MagentoLogger.PACKAGE_LOG_NAME```

- Regardless of the name, you can call ```MagentoLogger.get_package_handler()``` to retrieve the FileHandler that writes to it


The log level for console logging can be set with the ```stdout_level``` param (in multiple of the methods), but all log files are written to with ```DEBUG```

-  Note that Handler names incorporate the ```stdout_level``` into them, so a ```FileHandler``` may appear to not be ```DEBUG``` but trust me it is
